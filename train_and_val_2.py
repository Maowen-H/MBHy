...
class CovidDataset(Dataset):
    ...
    def get_sampler(self):
        labels = self.data['COVID_STATUS'].map({'n': 0, 'p': 1}).values
    ...
    def __getitem__(self, idx):
        ...
        id = row['SUB_ID']
        label = 1 if row['COVID_STATUS'] == 'p' else 0
...
def plot_training_metrics(writer, runs_dir,fold, system,plot_dir):
    """绘制训练过程中的指标"""
    # 从tensorboard日志中读取数据
    log_path = os.path.join(runs_dir, f'{system}_fold_{fold}')
    if not os.path.exists(log_path):
        print(f"Warning: Directory {log_path} does not exist")
        return
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, f'metrics_{system}_fold_{fold}.png'))
    plt.close()
def plot_confusion_matrix(y_true, y_pred, system, fold,plot_dir):
    """绘制混淆矩阵"""
    ...
    plt.savefig(os.path.join(plot_dir, f'confusion_matrix_{system}_fold_{fold}.png'))
    plt.close()

def train_and_validate_model(model, optimizer, train_loader, val_loader, epochs, model_name, class_weights, fold, trained_path,model_dir, log_dir, plot_dir, runs_dir):
    """训练并验证模型"""
    
    logger = setup_logger(f'{model_name}_fold_{fold}', os.path.join(log_dir, f'{model_name}_fold_{fold}.log'))
    pos_weight = torch.tensor([class_weights[1]/class_weights[0]]).to(device)
    criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5)
    writer = SummaryWriter(os.path.join(runs_dir, f'{model_name}_fold_{fold}'))
    patience = 10
    counter = 0
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter = 0
            torch.save(model.state_dict(), os.path.join(model_dir, f"{model_name}_fold_{fold}_best.pth"))
        else:
            counter += 1
            if counter >= patience:
                ...
                break
        # 训练结束后绘制图表
        plot_training_metrics(writer, runs_dir, fold, model_name, plot_dir)
        plot_confusion_matrix(metrics['y_true'], metrics['y_pred'], model_name, fold, plot_dir)
    writer.close()
