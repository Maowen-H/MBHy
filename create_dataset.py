...
# 配置logger
logging.basicConfig(
    filename='dataset_statistics.log',
    level=logging.INFO,
...
parser.add_argument("--image_path", help="Path to processed images", default="images")
...
def spectral_subtraction(audio):
    """频谱减法降噪"""
    noise_part = audio[:1000]
    noise_spectrum = np.mean(np.abs(librosa.stft(noise_part)))
